<div class="relative min-h-full space-y-8 rounded-xl bg-[#fffdf8] p-2 transition-opacity duration-200 sm:p-2"
    ...
    <span
        ...
        wire:loading wire:target="startDate,endDate">
        Updating...
    </span>
                ...
                ">
                    ...
                    <div class="mt-4 flex gap-2">
                        <a href="{{ route('transactions.create', ['type' => 'expense']) }}"
                            class="inline-flex items-center rounded-xl bg-red-700 px-3.5 py-2.5 text-sm font-semibold text-white transition hover:bg-red-800 focus:outline-none focus:ring-2 focus:ring-red-600/30">
                            <span class="mr-1.5" aria-hidden="true">+</span> Expense
                        </a>
                        <a href="{{ route('transactions.create', ['type' => 'income']) }}"
                            class="inline-flex items-center rounded-xl bg-emerald-700 px-3.5 py-2.5 text-sm font-semibold text-white transition hover:bg-emerald-800 focus:outline-none focus:ring-2 focus:ring-emerald-600/30">
    ...
    <section>
        ...
        <div class="overflow-hidden rounded-2xl border border-stone-200 bg-[#fbf8f2]">
            ...
            <article
                class="grid grid-cols-[auto_1fr_auto] items-center gap-x-3 gap-y-1 border-b border-stone-200 px-4 py-3.5 last:border-b-0 sm:grid-cols-[auto_minmax(0,1fr)_auto_auto] sm:px-5">
                <div
                    class="row-span-2 flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-semibold {{ $isTransfer ? 'bg-stone-100 text-stone-700' : ($isIncome ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700') }}">
                ...
                <p
                    class="self-end whitespace-nowrap text-right font-mono text-sm font-bold sm:col-start-4 sm:row-start-1 {{ $isTransfer ? 'text-stone-700' : ($isIncome ? 'text-emerald-700' : 'text-red-700') }}">
                    {{ $amountPrefix }}{{ number_format((float) ($transaction['amount'] ?? 0), 0) }}
                    <span class="hidden text-[10px] font-medium sm:inline">FCFA</span>
                </p>

                <time class="hidden whitespace-nowrap text-xs text-stone-500 sm:col-start-3 sm:row-start-1 sm:block"
